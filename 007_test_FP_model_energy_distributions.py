...
def extract_energy_distributions(n_rep, E_impact_eV_test, cos_theta_test, charge, mass):
    ...
    for i_ct, ct in enumerate(cos_theta_test):
        ...
        for etype in sey_mod.event_types.keys():
            etype_name = sey_mod.event_types[etype]
            hist, bin_edges = np.histogram(E_all_MPs_eV[event_type == etype], bins=n_bins, density=True)
            dists[etype_name].append((hist, bin_edges))
...
for i_ct, ct in enumerate(cos_theta_test):
    ...
    # wid = max(dists['true'][i_ct][1][:-1]) / len(dists['true'][i_ct][1][:-1])
    sp1.bar(dists['true'][i_ct][1][:-1], dists['true'][i_ct][0], color=thiscol, label=label, alpha=alpha)
    sp2.bar(dists['elast'][i_ct][1][:-1], dists['elast'][i_ct][0], color=thiscol, label=label, alpha=alpha)
    sp3.bar(dists['rediff'][i_ct][1][:-1], dists['rediff'][i_ct][0], color=thiscol, label=label, alpha=alpha)
    sp4.bar(dists['absorb'][i_ct][1][:-1], dists['absorb'][i_ct][0], color=thiscol, label=label, alpha=alpha)
