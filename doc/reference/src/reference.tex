...
\textbf{flag\_factor\_costheta} & If enabled, the true secondary emission yield is amplified depending on the impact angle relative to the surface normal. \\\hline
...
Still 'cosine\_2D' is kept as default for backwards compatibility.
\\ \hline
\end{longtable}
...
\begin{longtable}
    ...
    cen\_density($N\_{\mathrm{steps}}$)& [$\frac{1}{\mathrm{m}^3}$]& Electron density in a sphere around the center of the chamber with radius r\_center. \\\hline
    lam\_t\_array($N\_{\mathrm{steps}}$)& [?] & Current line density of the beam charge.\\\hline
\end{longtable}
...
\begin{longtable}
    ...
    \\\hline
    t\_hist($N\_{\mathrm{pass}}$) & [s] & The time where data for the bunch passages are saved.\\\hline
    t\_En\_hist($N\_{\mathrm{pass}}$) & [s] & Identical with t\_hist.\\\hline
    xg\_hist($N\_{\mathrm{bins}}$) & [m] & The $x$ coordinates corresponding to related histogram bins.\\\hline
    En\_hist($N_\mathrm{en}$) & [eV] & The energies corresponding to related histogram bins. \\\hline
    ...
    energ\_eV\_impact\_hist($N\_{\mathrm{pass}}$, $N\_{\mathrm{bins}}$ ) & & Electron impacts projected to the $x$-axis and sorted in bins (xg\_hist).\\\hline
    nel\_hist($N\_{\mathrm{pass}}$, $N\_{\mathrm{bins}}$ ) & & All electron positions projected to the $x$-axis and sorted in bins (xg\_hist).\\\hline
    nel\_impact\_hist\_scrub($N\_{\mathrm{pass}}$, $N\_{\mathrm{bins}}$ ) & & All electron impacts with an energy higher than scrub\_en\_th, projected to the $x$-axis and sorted in bins (xg\_hist).\\\hline
    nel\_impact\_hist\_tot($N\_{\mathrm{pass}}$, $N\_{\mathrm{bins}}$ ) & & All electron impacts, projected to the $x$-axis and sorted in bins.\\\hline
\end{longtable}
...
\begin{longtable}
    ...
    \\\hline
    dec\_fact\_out & & Output is only saved every nth time step, where n corresponds to dec\_fact\_out.\\\hline
    b\_spac & [s] & Bunch spacing used for simulation internals. \\\hline
    t\_sc\_video & [s] & The time whenever the space charge forces are recomputed. \\\hline
    U\_sc\_eV & & Not used. \\\hline
    N\_mp\_time & [s] & Only used when flag\_detailed\_MP\_info is set. Specifies the time the respective files are saved. Is this still active?\\\hline
    ...
    sec\_beam\_profiles & & Charge profile of secondary beams. \\\hline
    el\_dens\_at\_probes & & Electron densities at locations specified with el\_density\_probes. \\\hline
    ...
    r\_el\_dens\_probes & [m] & Radii taken into account to measure electron density probes \\\hline
