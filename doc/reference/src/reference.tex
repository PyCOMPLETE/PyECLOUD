...
The code provides the option of including more than one cloud in the simulation. Each cloud is influenced by the other clouds only through their space charge. This option is switched on by providing a list of additional cloud file names  in the standard input, as follows. % the following additional parameter in the standard input.
...
List of additional cloud file names.\\ \hline
\end{longtable}
...
In addition, the cloud file can contain the following optional parameters. Any optional parameter that is not specified in the cloud input file takes its value from the main input, and gets the default value if the parameter is not given in the main input files.

\begin{longtable}{p{0.42\textwidth}p{0.53\textwidth}}
\hline\endfirsthead\hline\endhead\rowcolor{Gray}
\multicolumn{2}{p{.97\textwidth}}{
\textbf{Optional cloud parameters}}\\ \hline
\textbf{MP management settings} & See Simulation parameters section.\\
\textbf{Saving settings} & \\
...
\textbf{Residual gas ionization parameters} & See Machine parameters section.\\ 
...
\begin{longtable}
    ...
    t\_En\_hist($N\_{\mathrm{pass}}$) & [s] & The time at which the electron energy spectra are saved.\\\hline
