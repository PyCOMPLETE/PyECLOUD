...
\item chamb\_type = `rectellip' for rect-elliptical chamber profile.
\end{enumerate}
\\ \hline
...
\\ \hline
\multicolumn{2}{p{.97\textwidth}}{When chamb\_type = `rectellip' the following input variables must be provided:}
\\ \hline
\textbf{filename\_chamb} & in format '[a,b,c,d]' - where a,b are parameters (semi-apertures) of the bounding rectangle and c,d  (semi-apertures) of the ellipse.
\\ \hline
\textbf{x\_aper, y\_aper} & Horizontal and vertical semi-apertures of the transverse chamber profile. Used for generating of initial uniform distribution of macro-particles (should be same as c,d parameters)
...
\item switch\_model = `FromFile' for the model loaded from the file (experimental data).
\end{enumerate}\\ \hline
\end{longtable}
...
\textbf{mufit}&Mu parameter of the lognormal distribution.\\ \hline
\multicolumn{2}{p{.97\textwidth}}{When switch\_model = `FromFile' the following input variables must be provided:}\\ \hline
\textbf{SEYFile} & Name of the file with SEY data (the format of the file are three columns with parameters: Energy,Reflected Part and SEY .\\ \hline
\end{longtable}
