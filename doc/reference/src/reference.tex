...
\textbf{Saving settings}
    ...
    \textbf{save\_mat\_every} & Output file .mat is saved only for every nth bunch passage. (optional - default 1) \\ \hline
...
Possible settings:
\begin{enumerate}
\item track\_method = `Boris' for a magnetic field loaded from a file.
...
\end{enumerate}
\\ \hline
\multicolumn{2}{p{.97\textwidth}}{When track\_method = `Boris' the following input variables must be provided:}
\\ \hline
\textbf{B\_map\_file} & Name of a .mat file containing the magnetic field map (x, y, Bx(x,y), By(x,y), Bz(x,y)).
...
\item   ''theta\_e\_max': float, value to which the incidence angle is limited when computing the angular dependence for backscattered electrons.
\end{enumerate}\\
\hline
...
\item	'r1': float, $r_1$.
\item	'r2': float, $r_2$.
\item   ''theta\_e\_max': float, value to which the incidence angle is limited when computing the angular dependence for rediffused electrons.
\end{enumerate}\\
\hline
