...
ref_folder = 'LHC_ArcDriftReal_450GeV_electron_n2_sey1.75_P3.125e+06'
...
for cloud_name in cloud_name_list:
    dict_ref = mlm.myloadmat(ref_folder + '/Pyecltest_angle%s_%s_ref.mat' % (args.angle_dist_func, cloud_name)) # load dictionary of the reference simulation
    dict_curr = mlm.myloadmat(curr_folder + '/Pyecltest_angle%s_%s.mat' % (args.angle_dist_func, cloud_name))   # load dictionary of the current simulation
    ...
    for ii, k in enumerate(out_var_curr):
        ...
        if len(dict_curr[k].shape) == 1:  # var is a vector!
            ...
            fig = pl.figure(i_fig + ii)
            fig.patch.set_facecolor('w')
            pl.subplots_adjust(right=0.75)
            pl.title(out_var_curr[ii] + ': ' + cloud_name, fontsize=titlesz)
            ...
            ms.sciy()
            pl.savefig(folder_plot + '/angle%s_%s_%s'%(args.angle_dist_func, cloud_name, k), dpi=300)
            ...
            pl.suptitle(out_var_curr[ii] + ': ' + cloud_name, fontsize=titlesz)
