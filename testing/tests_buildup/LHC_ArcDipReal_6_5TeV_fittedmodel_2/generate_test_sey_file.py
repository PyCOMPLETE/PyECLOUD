...
def test():
	...
	data_to_file = np.array([E_vect, ref_frac_vec_ecl, delta_vec_ecl]).T
	...
	sem_FromFile_obj = sem_FromFile.SEY_model_FromFile(fname, pyecl_input_folder='./')
	...
	pl.plot(E_vect, nel_emit_ec, '.g')
	pl.plot(E_vect, nel_emit_ff, 'xk')
	...
	for i_test in xrange(N_tests):
		nel_emit_ec, flag_elast_ec, flag_truesec_ec = sem_ECLOUD_obj.SEY_process(nel_impact=1.+E_vect*0., E_impact_eV=E_vect, costheta_impact=costheta_test+0*E_vect, i_impact=None)
		nel_emit_ff, flag_elast_ff, flag_truesec_ff = sem_ECLOUD_obj.SEY_process(nel_impact=1.+E_vect*0., E_impact_eV=E_vect, costheta_impact=costheta_test+0*E_vect, i_impact=None)
