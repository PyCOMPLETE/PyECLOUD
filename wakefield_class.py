...
class wakefield:
    ...
    def __init__(self, resistivity, b, x, y, alpha=0.05):
        self.e0 = sc.epsilon_0
        self.u0 = sc.mu_0
        self.resistivity = resistivity #sigma*m
        self.b = b #beam pipe radius (m)
        ...
        self.A = sc.elementary_charge/(2*np.pi*self.b)*np.sqrt(sc.c*self.resistivity)
        #self.y = np.zeros_like(self.x)
        self.x = x*sc.c  # z = s - ct
        self.y = y
        ...
        for i in range(len(self.y)):
            q = self.y[i] * dx  # y is charge over time need to be multiplied by time diff
    ...
    def E_r(self, x, y, z):
        ...
        #     func.append(yw_32[index(min(self.x-z))])
        func = np.interp(z, self.x, self.yw_52, left=0, right=0)
        return -3/4 * self.A * np.sqrt(x**2 + y**2)  * 1/np.sqrt(4*np.pi*self.e0) * func
    ...
    def B_theta(self, x, y, z):
        func = np.interp(z, self.x, self.yw_52, left=0, right=0)
        return -3/4*self.A*np.sqrt(x**2 + y**2) * func * np.sqrt(self.u0 / (4*np.pi))
