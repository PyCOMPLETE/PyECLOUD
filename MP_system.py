...
class MP_system:
    def __init__(self, N_mp_max, nel_mp_ref_0, fact_split, fact_clean,
        ...
        self.nel_mp_split = fact_split*self.nel_mp_ref;#can be optimized (only true second)
...
#                                                                                    bias_x_hist,Dx_hist,regen_hist_cut,Nxg_hist)
            ...
            bias_x=np.ceil(float(self.Nx_reg)/2.); #Attention when trnslating to python
            ...
            bias_vy=np.ceil(float(self.Nvy_reg)/2.); #Attention when trnslating to python
            
            Dvz_reg=2*vz_max/(self.Nvz_reg-1);
            bias_vz=np.ceil(float(self.Nvz_reg)/2); #Attention when trnslating to python
